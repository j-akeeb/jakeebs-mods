...
# Texture Overrides
TextureOverrides.registerTextureOverride(TextureOverrides::CHARS + "NPC jakeeb", TextureOverrides::MODBASE + "__SansX/jakeeb")
# Music Overrides
MusicOverrides.registerMusicOverride("Audio/BGM/jakeeb_sans", "Data/Mods/__SansX/jakeeb_sans")
MusicOverrides.registerMusicOverride("Audio/SE/jakeeb_drink", "Data/Mods/__SansX/jakeeb_drink")
# Map Patches
InjectionHelper.defineMapPatch(304) { |map| # Gearen Labs
  ...
  # Teleport to Xans HQ
  InjectionHelper.createNewEvent(map, 9, 12, "jakeeb_to_xans") { |event|
  # Initial interaction  
    event.newPage { |page|
...
InjectionHelper.defineMapPatch(308) { |map| # Removed Lab
  ...
  map.createSinglePageEvent(29, 43, "jakeeb_to_gearen") { |page|
    page.setGraphic("NPC jakeeb", direction: :Down, pattern: 0)
    page.interact(
      [:ShowText, "JAKEEB: Would you like to go back to Gearen Labs?"],
        [:ShowChoices, ["Yes", "No"], 2],
        [:When, 0, "Yes"],
          [:ShowText, "Right this way, then!"],
          [:PlaySoundEvent, "Entering Door", 80, 100],
          [:ChangeScreenColorTone, Tone.new(-255,-255,-255,0), 10],
          [:Wait, 10],
          [:TransferPlayer, :Constant, 304, 9, 14, :Down, true],
          [:ChangeScreenColorTone, Tone.new(0,0,0,0), 10],
          [:Wait, 10],
        :Done,
        [:When, 1, "No"],
  ...
  map.createSinglePageEvent(26, 40, "madame_xans") { |page|
    ...
    page.interact(
      [:Script, "TrainerRefightSelection.selectFight"],
      :Done
    )
  }
  # Heal Star
  ...
  map.createSinglePageEvent(26, 38, "meme_board") { |page|
    page.interact(
      [:ShowText, "It probably says something important, but unfortunately you're illiterate."],
      :Done
    )
  }
